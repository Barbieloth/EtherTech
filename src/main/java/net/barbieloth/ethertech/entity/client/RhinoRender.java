package net.barbieloth.ethertech.entity.client;

import com.mojang.blaze3d.vertex.PoseStack;
import net.barbieloth.ethertech.EtherTech;
import net.barbieloth.ethertech.entity.custom.RhinoEntity;
import net.minecraft.client.renderer.MultiBufferSource;
import net.minecraft.client.renderer.entity.EntityRendererProvider;
import net.minecraft.client.renderer.entity.MobRenderer;
import net.minecraft.resources.ResourceLocation;

public class RhinoRender extends MobRenderer<RhinoEntity, RhinoModel<RhinoEntity>> {
    public RhinoRender(EntityRendererProvider.Context pContext) {
        super(pContext, new RhinoModel<>(pContext.bakeLayer(ModModelLayers.RHINO_LAYER)), 2f);
    }

    @Override
    public ResourceLocation getTextureLocation(RhinoEntity pEntityw) {
        return new ResourceLocation(EtherTech.MOD_ID + ":textures/entity/rhino.png");
    }

    @Override
    public void render(RhinoEntity pEntity, float pEntityYaw, float pParticalTicks, PoseStack pMatrixStack, MultiBufferSource pBuffer, int pPackedLight) {

        if(pEntity.isBaby()){
            pMatrixStack.scale(0.5F, 0.5F, 0.5F);
        }

        super.render(pEntity,pEntityYaw,pParticalTicks,pMatrixStack,pBuffer,pPackedLight);
    }
}
